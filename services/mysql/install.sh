...
mysql_dbaas_username=#{mysql_dbaas_username}
mysql_dbaas_password=#{mysql_dbaas_password}
mysql_activiti_username=#{mysql_activiti_username}
mysql_activiti_password=#{mysql_activiti_password}
mysql_scheduler_username=#{mysql_scheduler_username}
mysql_scheduler_password=#{mysql_scheduler_password}
mysql_monitormanager_username=#{mysql_monitormanager_username}
mysql_monitormanager_password=#{mysql_monitormanager_password}
mysql_taskmanagement_username=#{mysql_taskmanagement_username}
mysql_taskmanagement_password=#{mysql_taskmanagement_password}
...
function __initZDataConfig {
  if [[ ${installType} == "1" ]];then
    ...
    mysql -uroot -p${mysqlpassword} -h${mysqlhost} -P${mysqlhostport} < dbsqlfile/init_zData_config.sql >> ${logFile} 2>&1
  fi
}
...
function __StartInitMySQLNonRoot {
      ...
      if [[ ${retCode} == 0 ]]; then
        touch .mysqlinit
        if [[ ${theme} != "zData" ]];then
          dbaas_username=(${mysql_dbaas_username})
          dbaas_paasword=(${mysql_dbaas_password})
          activiti_username=(${mysql_activiti_username})
          activiti_paasword=(${mysql_activiti_password})
          scheduler_username=(${mysql_scheduler_username})
          scheduler_paasword=(${mysql_scheduler_password})
          monitormanager_username=(${mysql_monitormanager_username})
          monitormanager_paasword=(${mysql_monitormanager_password})
          taskmanagement_username=(${mysql_taskmanagement_username})
          taskmanagement_password=(${mysql_taskmanagement_password})
