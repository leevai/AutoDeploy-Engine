...
function __CheckNginx {
    #nginx WEB只在节点1安装
    if [[ $( __readINI nodeconfig/current.cfg service dbaas-web ) == ${nodeNum}  && ${nodeNum} == 1 ]]; then
        set +e
...
function __InstallNginx {
    ...
    tar -zxf soft/nginx/nginx-1.22.0.tar.gz -C "${installPath}/soft/nginx/"
    tar -zxf soft/nginx/pcre-8.43.tar.gz -C "${installPath}/soft/nginx/"
    tar -zxf soft/nginx/zlib-1.2.12.tar.gz -C "${installPath}/soft/nginx/"
    tar -zxf soft/nginx/v0.33.tar.gz -C "${installPath}/soft/nginx/"
    tar -zxf soft/nginx/openssl-1.1.1q.tar.gz -C "${installPath}/soft/nginx/"

    info "Install pcre..."
    info "Install pcre log location ${installPath}/soft/nginx/pcre_install.log"
    cd ${installPath}/soft/nginx/pcre-8.43
    info "Current directory `pwd`"
    ./configure --prefix=${installPath}/soft/nginx/nginx-1.22.0/lib/pcre > ../pcre_install.log 2>&1
     make >> ../pcre_install.log 2>&1 && make install >> ../pcre_install.log 2>&1

    info "Install zlib..."
    info "Install pcre log location ${installPath}/soft/nginx/pcre_install.log"
    cd ../zlib-1.2.12
    info "Current directory `pwd`"
    ./configure --prefix=${installPath}/soft/nginx/nginx-1.22.0/lib/zlib > ../zlib_install.log 2>&1
    make >> ../zlib_install.log 2>&1 && make install >> ../zlib_install.log 2>&1

    info "Compiling and install Nginx..."
    info "Compiling and install Nginx log location ${installPath}/soft/nginx/nginx_install.log"
    mkdir -p ${installPath}/soft/nginx/nginx
    cd ../nginx-1.22.0
    set +e
    ./configure\
         --prefix=${installPath}/soft/nginx/nginx\
         --add-module=${installPath}/soft/nginx/headers-more-nginx-module-0.33\
         --with-http_stub_status_module\
         --with-mail\
         --with-stream\
         --with-http_ssl_module\
         --with-pcre=${installPath}/soft/nginx/pcre-8.43\
         --with-zlib=${installPath}/soft/nginx/zlib-1.2.12\
         --with-openssl=${installPath}/soft/nginx/openssl-1.1.1q >> ../nginx_install.log 2>&1
         make >> ../nginx_install.log 2>&1 && make install >> ../nginx_install.log 2>&1
         retCode=$?
    set -e
    info "指定openssl编译返回码${retCode}"
    if [[ ${retCode} != 0 ]];then
    info "指定openssl编译失败，尝试不指定openssl编译"
          ./configure\
               --prefix=${installPath}/soft/nginx/nginx\
               --add-module=${installPath}/soft/nginx/headers-more-nginx-module-0.33\
               --with-http_stub_status_module\
               ...
               --with-http_ssl_module\
               --with-pcre=${installPath}/soft/nginx/pcre-8.43\
               --with-zlib=${installPath}/soft/nginx/zlib-1.2.12 >> ../nginx_install.log 2>&1
          make >> ../nginx_install.log 2>&1 && make install >> ../nginx_install.log 2>&1
    fi
...
function __LowCodeWorkFlowConfig {
  ...
  if [[ ${installNodeType} == "OneNode" ]]; then
    lowcodeIp=${realHostIp}
  else
    lowcodeIp=$( __readINI zcloud.cfg multiple workflow.ip )
  fi
